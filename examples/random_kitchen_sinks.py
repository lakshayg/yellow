...
try:
    ...
    # train a random kitchen sinks features
    if 1:
        T1 = []
        T2 = []
        Acc = []
        for i in range(500):
            X_train, X_test, y_train, y_test = train_test_split(X, y)
            sampler = RBFSampler(gamma=1, n_components=500)
            clf = svm.LinearSVC(max_iter=1)
            ...
            print_stats('RKS-SVM {}'.format(i), t1, t2, time(), y_test, y_pred)
        print "mean train time:", np.mean(T1)
        print "mean test  time:", np.mean(T2)
        print "mean accuracy  :", np.mean(Acc)
