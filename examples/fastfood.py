...
from yellow.feature_map import Fastfood
...
try:
    ...
    if 1:
        ...
        for i in range(500):
            X_train, X_test, y_train, y_test = train_test_split(X, y)
            sampler = Fastfood(sigma=1, tradeoff_mem_accuracy='mem', n_components=2)
            ...
            print_stats('fastfood-SVM {}'.format(i), t1, t2, time(), y_test, y_pred)
        print "mean train time:", np.mean(T1)
        print "mean test  time:", np.mean(T2)
        print "mean accuracy  :", np.mean(Acc)
